  ...
  if (!product) {
    return (
      <div className="container-main mx-auto px-4 py-20 text-center">
        ...
        <Link to="/catalog"><Button variant="outline" className="mt-4"><ArrowLeft className="w-4 h-4 mr-1" /> Back to Catalog</Button></Link>
      </div>
    );
  }
  ...
  return (
    <div className="container-main mx-auto px-4 py-8">
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground mb-6">
        ...
        <ChevronRight className="w-3 h-3" />
        <Link to={`/catalog?category=${product.category}`} className="hover:text-foreground capitalize">{product.category.replace('-', ' ')}</Link>
        <ChevronRight className="w-3 h-3" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Left: Image */}
        <div className="bg-card border border-border/50 rounded-2xl p-8 flex items-center justify-center min-h-[300px] lg:min-h-[400px]">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-4 mt-4 text-xs text-muted-foreground">
            ...
            <span className="flex items-center gap-1"><CheckCircle2 className="w-3.5 h-3.5 text-primary" /> Verified product</span>
            {product.stock < 10 && <span className="flex items-center gap-1 text-warning">⚠️ Only {product.stock} left</span>}
          </div>
        ...
        {activeTab === 'description' && (
          <div className="max-w-3xl space-y-4">
            ...
            <div className="p-4 bg-card border border-border/50 rounded-xl">
              <h4 className="font-display font-semibold text-sm mb-2">Delivery Information</h4>
              <p className="text-sm text-muted-foreground">
                {product.deliveryType === 'instant'
                  ? 'This product is delivered instantly after payment confirmation. You will receive your credentials via email within minutes.'
                  : 'This product requires manual processing. Delivery typically takes 1-24 hours after payment confirmation.'}
              </p>
            </div>
            <div className="p-4 bg-card border border-border/50 rounded-xl">
              <h4 className="font-display font-semibold text-sm mb-2">Guarantee & Replacement</h4>
              <p className="text-sm text-muted-foreground">{product.guarantee}. If the product doesn't match the description or stops working within the guarantee period, we'll provide a free replacement or full refund.</p>
            </div>
          </div>
        )}
          ...
          <div className="max-w-2xl">
            <div className="bg-card border border-border/50 rounded-xl overflow-hidden">
              {Object.entries(product.specifications).map(([key, value], i) => (
                <div key={key} className={`flex items-center justify-between px-4 py-3 text-sm ${i % 2 === 0 ? 'bg-secondary/30' : ''}`}>
                ...
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      {review.verified && <span className="text-[10px] text-primary ml-2">✓ Verified</span>}
                    </div>
                  </div>
      ...
      {similar.length > 0 && (
        <section className="mt-16">
          <h2 className="font-display text-xl font-bold mb-6">Similar Products</h2>
      ...
      {/* Support CTA */}
      <div className="mt-12 p-6 bg-card border border-border/50 rounded-xl flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <MessageCircle className="w-8 h-8 text-primary" />
          <div>
            <h4 className="font-display font-semibold">Have questions about this product?</h4>
            <p className="text-sm text-muted-foreground">Our support team can help you 24/7</p>
          </div>
        </div>
