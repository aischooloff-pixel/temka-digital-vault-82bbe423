  ...
  return (
    <div className="group relative bg-card border border-border/50 rounded-xl overflow-hidden hover-lift hover:border-primary/30 transition-all duration-300">
      ...
      <Link to={`/product/${product.id}`} className="block">
        <div className="relative h-44 bg-secondary/50 flex items-center justify-center overflow-hidden">
          ...
          <div className="absolute top-2 left-2 flex flex-wrap gap-1">
            {product.tags.map(tag => (
              <span key={tag} className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${tagColors[tag] || 'bg-secondary text-secondary-foreground border-border'}`}>
      ...
      <div className="p-4">
        ...
        <Link to={`/product/${product.id}`}>
          <h3 className="font-display font-semibold text-sm mt-1 line-clamp-1 group-hover:text-primary transition-colors">{product.title}</h3>
        </Link>
        ...
        <div className="flex items-center gap-1 mt-2">
          {product.deliveryType === 'instant' ? (
            ...
            <span className="flex items-center gap-1 text-[10px] text-muted-foreground font-medium">
              <Clock className="w-3 h-3" /> Manual delivery
            </span>
          )}
          {product.stock < 10 && (
            <span className="text-[10px] text-warning font-medium ml-auto">Only {product.stock} left</span>
          )}
        </div>
        ...
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-border/30">
          <div>
            ...
            {product.oldPrice && (
              <span className="text-xs text-muted-foreground line-through ml-2">${product.oldPrice}</span>
            )}
          </div>
